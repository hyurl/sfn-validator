...
function parseRule(rules: { [field: string]: string | Rule }, prefix = "") {
    ...
    for (let k in rules) {
        ...
        } else if (rule.equals && (!rules[rule.equals] || !(<Rule>rules[rule.equals]).required)) {
...
function filter(rules: { [field: string]: Rule }, data: object) {
    var _data: object = {};

    for (let k in data) {
        if (k in rules) {
            if (typeof rules[k] === "object" && rules[k].type == "object")
                _data[k] = filter(<any>rules[k].children, data[k]);
            else
                _data[k] = data[k];
        }
    }
...
function validate(rules: { [field: string]: Rule }, data: object, prefix = "") {
    var strings = ["string", "email", "url", "ascii", "base64", "json"];

    for (let k in rules) {
        let rule: Rule = rules[k];
        let matches: RegExpMatchArray;
        let msg: string;
        let throwTypeError = (msg: string) => {
        ...
        switch (rule.type) {
            ...
            case "ipv4":
                if (!IsIp.v4(data[k])) {
                    ...
                    let parts: number[] = data[k].split(".").map(v => parseInt(v));
        ...
        if ((rule.type === "array" || strings.includes(rule.type)) && rule.length) {
            ...
            // Walk object.
            validate(<any>rule.children, data[k], `${prefix}${k}.`);
        }
    }
    return true;
}

function checkBasicType(rule: Rule, value: any, prefix: string) {
 ...
 */
class Validator {
    rules: { [field: string]: Rule };

    /**
     * Creates a validator with specified rules.
     */
    constructor(rules: { [field: string]: string | Rule }) {
        if (rules)
            this.set(rules);
    }

    /** (**deprecated**) Sets the validating rules. */
    set(rules: { [field: string]: string | Rule }) {
